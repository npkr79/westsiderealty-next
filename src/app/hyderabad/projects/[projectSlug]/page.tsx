// Wrapper route for /hyderabad/projects/[projectSlug]
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  try {
    ...
    const project = await projectService.getCityLevelProjectBySlug(CITY_SLUG, projectSlug);

    if (!project) {
      return { title: "Project Not Found" };
    }

    const canonicalUrl = `https://www.westsiderealty.in/${CITY_SLUG}/projects/${projectSlug}`;
    const cityName = project.city?.city_name || CITY_SLUG;
...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  if (project?.enable_intelligence === true) {
    const intelligenceData =
      await projectIntelligenceService.getProjectIntelligenceBySlug(CITY_SLUG, projectSlug);
    return (
      <ProjectIntelligenceTemplate project={project} intelligenceData={intelligenceData} />
    );
  }
  
  console.log(`[ProjectPage] citySlug=${CITY_SLUG}, projectSlug=${projectSlug}, found=${!!project}`);
  ...
  const cityName = cityData?.city_name || CITY_SLUG;
  ...
  const breadcrumbItems = [
    { name: "Home", href: "/" },
    { name: cityName, href: `/${CITY_SLUG}` },
    ...(microMarketSlug && microMarket
      ? [{ name: microMarket.micro_market_name || microMarketSlug, href: `/${CITY_SLUG}/${microMarketSlug}` }]
      : []),
    { name: "Projects", href: `/${CITY_SLUG}/projects` },
    { name: project.project_name, href: `/${CITY_SLUG}/projects/${projectSlug}` },
  ];
  ...
  const unifiedSchema = generateUnifiedSchema({
    ...
    breadcrumbs: [
      ...
      { name: cityName, item: `https://www.westsiderealty.in/${CITY_SLUG}` },
      ...(microMarketSlug && microMarket
        ? [{ name: microMarket.micro_market_name || microMarketSlug, item: `https://www.westsiderealty.in/${CITY_SLUG}/${microMarketSlug}` }]
        : []),
      { name: "Projects", item: `https://www.westsiderealty.in/${CITY_SLUG}/projects` },
