...
export default function ProjectDetailClient(props: { citySlug: string; projectSlug: string }) {
  const { citySlug, projectSlug } = props;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const fetchPromise = projectService.getCityLevelProjectBySlug(
          citySlug,
          projectSlug
        );

        const projectData = await Promise.race([fetchPromise, timeoutPromise]);
        ...
        if (projectData) {
          ...
          findBrochureByProjectName(projectData.project_name)
            .then(url => {
              if (isMounted) setBrochureUrl(url);
            })
          ...
          console.warn(`[ProjectDetailPage] Project ${projectSlug} not found for city ${citySlug}`);
        }
      } catch (error) {
