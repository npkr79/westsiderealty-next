...
export default function CityPage() {
  ...
  const slug = citySlug || location.pathname.split("/").filter(Boolean)[0] || "hyderabad";
  const [city, setCity] = useState<CityInfo | null>(null);
  const [featuredProjects, setFeaturedProjects] = useState<ProjectInfo[]>([]);
  const [featuredGoaProperties, setFeaturedGoaProperties] = useState<GoaHolidayProperty[]>([]);
  const [microMarkets, setMicroMarkets] = useState<MicroMarketGridItem[]>([]);
  ...
  useEffect(() => {
    const loadCityData = async () => {
      ...
      if (cityData) {
        ...
        if (cityData.city_name.toLowerCase() === 'goa') {
          // Fetch featured Goa properties
          const goaProperties = await goaHolidayPropertyService.getProperties({ 
            is_featured: true,
            status: 'Active'
          });
          setFeaturedGoaProperties(goaProperties.slice(0, 6));
        } else {
          // Fetch featured projects for other cities
          const projects = await projectService.getProjectsByCity(cityData.id, true);
          ...
          setMicroMarkets(markets);

          // Fetch total property count for Hyderabad
          if (slug === 'hyderabad') {
            const listingsCount = await cityService.getPublishedPropertyCount(slug);
  ...
  // Static SEO for loading state (ensures Googlebot sees content)
  const loadingCanonicalUrl = `https://www.westsiderealty.in/${slug}`;
  
  if (isLoading) {
    return (
      <>
        <Helmet>
          <title>{slug === 'hyderabad' ? 'Real Estate Investment in Hyderabad | RE/MAX Westside Realty' : `Properties in ${slug} | RE/MAX Westside`}</title>
          <meta name="description" content={slug === 'hyderabad' ? 'Discover premium real estate in Hyderabad. Explore luxury apartments, villas, and investment properties in top micro-markets like Neopolis, Kokapet, and Financial District.' : `Find premium properties in ${slug}`} />
          <link rel="canonical" href={loadingCanonicalUrl} />
        </Helmet>
  ...
  // Generate canonical URL - always use www version per Google Search Console preference
  const canonicalUrl = city.canonical_url || `https://www.westsiderealty.in/${slug}`;
  
  // Generate JSON-LD structured data for the city page
  const cityJsonLd = {
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta property="og:image" content={city.hero_image_url || hyderabadHero} />
        ...
        <meta name="twitter:image" content={city.hero_image_url || hyderabadHero} />
        
        {/* JSON-LD Structured Data */}
        <script type="application/ld+json">
          {JSON.stringify(cityJsonLd)}
        </script>
        <script type="application/ld+json">
      ...
      <section className="relative py-32 px-4 overflow-hidden min-h-[70vh] flex items-center">
        ...
        <div 
          ...
          style={{ backgroundImage: `url(${city.hero_image_url || hyderabadHero})` }}
        />
        ...
        <div className="container mx-auto relative z-10">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <div className="flex flex-wrap gap-4 justify-center pt-6">
              <Button size="lg" className="text-base px-8 shadow-lg hover:shadow-xl transition-shadow bg-primary hover:bg-primary/90" asChild>
                <Link to={city.city_name.toLowerCase() === 'goa' ? '/goa/properties' : '/properties'}>
      ...
      {slug === "hyderabad" && (
        <MarketUpdateBanner citySlug={slug} />
      )}
      ...
      {(city.average_price_sqft || city.annual_appreciation_pct || city.rental_yield_pct) && (
        <section className="py-16 bg-secondary/10">
          <div className="container px-4">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-[hsl(var(--heading-blue))]">Market Snapshot</h2>
      ...
      {city.lifestyle_infrastructure_json && (
        <CityLifestyleHub 
          lifestyleData={city.lifestyle_infrastructure_json as any}
      ...
      {city.buyer_personas_json && (
        <BuyerPersonasSection 
          personas={city.buyer_personas_json}
          cityName={city.city_name}
        />
      )}
      ...
      {/* Micro-Markets Grid - NEW SECTION */}
      ...
      <InvestmentAreasSection 
        cityName={city.city_name}
        investmentZonesData={city.investment_zones_json}
      />
          ...
          <div className="container px-4">
            ...
            <div className="flex justify-center mt-8">
              <Button variant="outline" asChild>
                <Link to="/goa/properties">View All Properties</Link>
              </Button>
            </div>
          </div>
        </section>
