...
const getCanonicalUrl = (pageData: MicroMarketPage | null, slug: string | undefined): string => {
  if (pageData?.canonical_url) {
    return pageData.canonical_url;
  }
  // Fallback: Construct canonical URL with www version per Google Search Console preference
  return `https://www.westsiderealty.in/hyderabad/${slug}`;
};
...
const highlightMetrics = (html: string): string => {
  ...
  let highlighted = html.replace(
    /(\d+(?:\.\d+)?(?:-\d+(?:\.\d+)?)?%)/g, 
    '<span class="metric-positive">$1</span>'
  );
...
const getFaqSchemaJsonString = (pageData: MicroMarketPage | null): string => {
  ...
  if (pageData.faq_schema_json) {
    ...
    // to remove SQL escaping before final stringification.
    if (typeof schemaData === 'string') {
      try {
        schemaData = JSON.parse(schemaData);
      } catch (e) {
        // If parsing fails, fall through to fallback generation
        schemaData = null;
      }
    }
  } 
...
export default function MicroMarketPageComponent() {
  const { microMarketSlug: slug } = useParams<{ microMarketSlug: string }>();
  const [pageData, setPageData] = useState<MicroMarketPage | null>(null);
  const [featuredProjects, setFeaturedProjects] = useState<FeaturedProject[]>([]);
  const [loading, setLoading] = useState(true);
  const [microMarketProjects, setMicroMarketProjects] = useState<ProjectWithRelations[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (data) {
        setPageData(data);
        const projects = await microMarketPagesService.getFeaturedProjectsForPage(data.id);
        setFeaturedProjects(projects);
        
        // Fetch projects for this micro-market
        setLoadingProjects(true);
        const mmProjects = await projectService.getProjectsByMicroMarket('hyderabad', slug);
        
        // For Neopolis, show all projects (12); for other micro-markets, limit to 9 random projects
        if (slug.toLowerCase() === 'neopolis') {
          setMicroMarketProjects(mmProjects);
        } else {
          // Shuffle and limit to 9 random projects
          const shuffled = [...mmProjects].sort(() => Math.random() - 0.5);
          setMicroMarketProjects(shuffled.slice(0, 9));
        }
  ...
  // Generate static schemas for Neopolis even during loading (for SEO crawlers)
  const isNeopolisPage = slug?.toLowerCase() === 'neopolis';
  const staticBreadcrumbSchema = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://www.westsiderealty.in" },
      { "@type": "ListItem", "position": 2, "name": "Hyderabad", "item": "https://www.westsiderealty.in/hyderabad" },
      { "@type": "ListItem", "position": 3, "name": isNeopolisPage ? "Neopolis" : (slug || ""), "item": `https://www.westsiderealty.in/hyderabad/${slug}` }
    ]
  };

  const staticRealEstateSchema = isNeopolisPage ? {
    "@context": "https://schema.org",
    "@type": "RealEstateListing",
    "name": "Properties in Neopolis, Hyderabad",
    "description": "Discover Neopolis, Hyderabad's premier ultra-luxury hub. Find high-rise apartments and sky-villas near the Financial District.",
    "url": "https://www.westsiderealty.in/hyderabad/neopolis",
    "address": { "@type": "PostalAddress", "addressLocality": "Neopolis", "addressRegion": "Hyderabad", "addressCountry": "IN" },
    "offers": { "@type": "AggregateOffer", "priceCurrency": "INR", "lowPrice": 12000, "highPrice": 15000 },
  ...
  if (loading) {
    return (
      <>
        <Helmet>
          <title>{isNeopolisPage ? "Neopolis, Kokapet: Ultra-Luxury Apartments & Investment Guide | RE/MAX Westside" : `${slug} | RE/MAX Westside`}</title>
          <meta name="description" content={isNeopolisPage ? "Discover Neopolis, Hyderabad's premier ultra-luxury hub. Find high-rise apartments and sky-villas near the Financial District." : `Properties in ${slug}, Hyderabad`} />
          <link rel="canonical" href={`https://www.westsiderealty.in/hyderabad/${slug}`} />
          <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(staticBreadcrumbSchema) }} />
          {staticRealEstateSchema && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(staticRealEstateSchema) }} />}
        </Helmet>
  ...
  const isNeopolis = slug?.toLowerCase() === 'neopolis';
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": "Hyderabad",
        "item": "https://www.westsiderealty.in/hyderabad"
      },
      {
        "@type": "ListItem",
        "position": 3,
  ...
  const realEstateListingSchema = {
    "@context": "https://schema.org",
    "@type": "RealEstateListing",
    "name": `Properties in ${pageData.micro_market_name}, Hyderabad`,
    ...
    ...(pageData.price_per_sqft_min && pageData.price_per_sqft_max && {
      "offers": {
        "@type": "AggregateOffer",
        "priceCurrency": "INR",
  ...
  return (
    <Layout>
      <Helmet>
        ...
        {pageData.hero_image_url && <meta property="og:image" content={pageData.hero_image_url} />}
        
        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={seoTitle} />
        <meta name="twitter:description" content={seoDescription} />
        {pageData.hero_image_url && <meta name="twitter:image" content={pageData.hero_image_url} />}
        ...
        {/* FAQ Schema - Injection using robust helper function */}
        {getFaqSchemaJsonString(pageData) && (
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <section className="mb-12 mt-8">
            {pageData.hero_image_url && (
              <div className="mb-6 rounded-lg overflow-hidden">
                <img 
                  ...
                  alt={`Aerial view of ${pageData.micro_market_name} ultra-luxury residential township in ${pageData.key_adjacent_areas?.[0] || 'West Hyderabad'}, Hyderabad`}
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold">₹{pageData.price_per_sqft_min?.toLocaleString()} - ₹{pageData.price_per_sqft_max?.toLocaleString()}/sqft</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-bold">{pageData.annual_appreciation_min}% - {pageData.annual_appreciation_max}%</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-bold">{pageData.rental_yield_min}% - {pageData.rental_yield_max}%</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            {/* Connectivity Map - Compact, after info cards */}
          ...
          {/* Market Update Banner - After Connectivity Map */}
          <MarketUpdateBanner citySlug="hyderabad" microMarketSlug={slug} />

          {/* Authority Content Block - Premium Editorial Section */}
          {pageData.mm_authority_content && (
            <section className="mb-8">
              <div 
                className="authority-content-block"
                dangerouslySetInnerHTML={{ __html: pageData.mm_authority_content }}
              />
            </section>
          )}

          {/* Quick Facts Module - New SEO Section */}
          <QuickFactsModule
          ...
          {pageData.master_plan_json && (
            <MasterPlanSection data={pageData.master_plan_json} />
          )}
          ...
          {/* Strategic Infrastructure & Social Amenities - New SEO Section */}
          <StrategicInfrastructureSection
            microMarketName={pageData.micro_market_name}
            nearestMmtsStatus={pageData.nearest_mmts_status}
          />

        {/* Quick-Reference Project Table - Using microMarketProjects */}
        {microMarketProjects.length > 0 && (
          <section className="mb-12">
            <h2 className="micro-market-h2">
              Featured Projects in {pageData.micro_market_name}
            </h2>
            
            {/* DATA VISUALIZATION - Price Trend Chart (Optional) */}
            {pageData.market_analysis_chart_url && (
              <div className="chart-container mb-8">
                ...
                <img 
                  src={pageData.market_analysis_chart_url} 
              ...
              <p className="text-muted-foreground mb-6">
                We specialize in showcasing the finest properties in the region. Here is a quick, 
                at-a-glance overview of some of the key <strong className="metric-highlight">{pageData.micro_market_name} projects</strong> currently 
                listed with us, demonstrating the variety of luxury and premium inventory available:
              </p>
                      ...
                      <TableBody>
                        {microMarketProjects.map((project) => (
                          <TableRow key={project.id}>
                            <TableCell>
                              <Link 
                                to={`/hyderabad/${pageData.url_slug}/projects/${project.url_slug}`}
                            ...
                            <TableCell>
                              {project.developer?.url_slug ? (
                                <Link 
                                  to={`/developers/${project.developer.url_slug}`}
                ...
                <CardContent className="pt-6">
                  ...
                  {pageData.filtered_properties_url && (
                    <div className="flex justify-center">
                      <Link to={pageData.filtered_properties_url}>
          ...
          {pageData.developer_pillar_urls && Object.keys(pageData.developer_pillar_urls).length > 0 && (
            <section className="mb-12">
              ...
              <p className="text-muted-foreground mb-8">
                ...
                The presence of these top-tier national and regional builders confirms the area's high-value proposition, 
                driven by quality construction and strategic location.
              </p>
                      ...
                      <CardHeader>
                        <CardTitle>
                          <div className="flex items-center gap-4">
                            ...
                            {/* TODO: Re-enable when developer pages go live */}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                          {developer.projects.map((project, idx) => (
                            <div key={idx} className="block">
                              {/* TODO: Re-enable when project pages go live */}
                              <Button 
                                variant="outline" 
                                className="w-full h-auto py-4 px-4 flex flex-col items-center gap-2 cursor-default opacity-90"
                                disabled
                              >
                                ...
                                {project.status === 'under-construction' && (
                                  <Badge variant="secondary" className="text-xs">U/C</Badge>
                                )}
                  ...
                  <Card key={project.id}>
                    ...
                    <CardContent>
                      {project.project_summary && (
                        <p className="text-muted-foreground mb-4">{project.project_summary}</p>
                      )}
                      {/* TODO: Re-enable when project pages go live */}
                    ...
                    <CardContent>
                      <ul className="space-y-2">
                        {pageData.top_schools.map((school, idx) => (
                          <li key={idx} className="text-muted-foreground">• {school}</li>
                        ))}
                      </ul>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <ul className="space-y-2">
                        {pageData.top_hospitals.map((hospital, idx) => (
                          <li key={idx} className="text-muted-foreground">• {hospital}</li>
                        ))}
                      </ul>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <ul className="space-y-2">
                        {pageData.entertainment_centers.map((center, idx) => (
                          <li key={idx} className="text-muted-foreground">• {center}</li>
                        ))}
                      </ul>
                    </CardContent>
                  </Card>
          ...
          <section className="mb-12">
            <h2 className="micro-market-h2">
              Explore Projects in {pageData.micro_market_name}
            </h2>
            <p className="text-muted-foreground mb-8 max-w-3xl">
              Discover premium residential projects in {pageData.micro_market_name}. 
              Each project offers world-class amenities and excellent investment potential.
            </p>
            ...
            {loadingProjects ? (
              ...
              <p className="text-muted-foreground text-center py-8">
                No projects found in this micro-market.
              </p>
            )}
          </section>
                ...
                <CardContent className="pt-6">
                  <Accordion type="single" collapsible className="w-full">
                    {pageData.faqs.map((faq: any, idx) => {
                      // Handle both q/a and question/answer formats
      ...
      <CityHubBacklink citySlug="hyderabad" cityName="Hyderabad" />
    </Layout>
  );
}
