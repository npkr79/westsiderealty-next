...
export default async function SmartLinkGrid({
  ...
  try {
    ...
    console.log('[SmartLinkGrid] Stats for', microMarketName, ':', {
      microMarketId,
      cityId,
      totalProjects: stats.totalProjects,
      residentialTypes: stats.residentialTypes,
      commercialTypes: stats.commercialTypes,
    ...
    if (residentialLinks.length === 0 && commercialLinks.length === 0 && priceLinks.length === 0 && statusLinks.length === 0) {
      if (process.env.NODE_ENV === 'development') {
        console.log('[SmartLinkGrid] No links generated for', microMarketName);
      }
      return null;
    }
