    ...
    const load = async () => {
      ...
      try {
        ...
        const mappedLeads: LeadRow[] = ((leadsRes.data as Row[]) || []).map((row) => ({
          ...
          source: asText(row.source_name) || asText(row.source_id) || "unknown",
          agentId: asText(row.assigned_to) || asText(row.assigned_agent_id) || "unassigned",
        ...
        const mappedDeals: DealRow[] = ((dealsRes.data as Row[]) || []).map((row) => ({
          ...
          assignedTo: asText(row.assigned_to) || asText(row.owner_id) || asText(row.assigned_agent_id) || "unassigned",
          createdAt: asDate(row.created_at),
